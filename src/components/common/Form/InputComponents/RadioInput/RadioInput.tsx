...
const RadioInput: FC<RadioInputType> = (field) => {
	const items = field.items?.map((item) => {
		if (typeof item !== 'string') return <FormControlLabel key={item.name} value={item.value} control={<Radio />} label={item.name} />;
		return <FormControlLabel key={item} value={item} control={<Radio />} label={item} />;
	});
	const { helperText } = field;

	return (
		<FormControl required={field.required} error={field.error} disabled={field.disabled}>
			<FormLabel id={field.id} onChange={field.onChange} onBlur={field.onBlur}>
				{field.label}
			</FormLabel>
