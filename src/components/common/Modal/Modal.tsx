...
import { UseModalState } from './ModalReducer';
import ActionButton from '../ActionButton/ActionButton';

interface ModalProps {
	state: UseModalState[0];
	dispatches: UseModalState[1];
}

export default function Modal({ state, dispatches }: ModalProps) {
                    ...
                    hover:bg-gray-50 focus:outline-none focus:ring-2
                     focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0
                      sm:ml-3 sm:w-auto sm:text-sm"
										onClick={() => dispatches.setClose()}
										ref={cancelButtonRef}
									>
										Cancel
									</button>
								)}
								{state.actionButton && <ActionButton {...state.actionButton} buttonRef={state.buttonRef} onClick={dispatches.setClose} />}
							</div>
						</div>
					</Transition.Child>
				</div>
